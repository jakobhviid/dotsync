...
| `dotsync adopt <paths…> [--group <name>] [--mac\|--linux] [--dry-run]` | Move existing `$HOME` files/dirs into the cloud folder and symlink them back. Chain several paths to adopt them together. Every mapping belongs to a group: `--group` sets it, otherwise it's derived from the path (interactive picker on a terminal, auto-derived and echoed non-interactively). `--mac`/`--linux` scopes to one OS; `--dry-run` previews without moving. |
| `dotsync install [names…] [--all] [--dry-run] [--adopt]` | Link mappings on this machine. No args on a terminal opens the picker; `--all` links everything applicable. A name matches a group (all its members) or a single mapping. `--adopt` resolves conflicts by letting the cloud copy win, backing the local file up to `.bak` (a one-shot `on_conflict = "adopt"`). |
| `dotsync uninstall [names…] [--all] [--dry-run]` | Remove dotsync's symlinks here (the cloud copies stay; the mappings stay). |
| `dotsync unadopt <names…> [--dry-run] [--yes]` | Stop syncing one or more mappings **everywhere**: restore them to `$HOME` here (cloud copies kept) and drop them from `dotsync.toml` on all machines. Needs a confirm (or `--yes`). |
...
| `dotsync doctor [--fix]` | Detect atomic-save clobbers, conflicts, dangling/foreign links, secret-mode drift, a secret or the sync folder **inside a git repo** (an error — git could commit it), a world-readable sync folder holding secrets, cloud conflict copies, and cross-machine drift (orphan symlinks whose mapping was removed elsewhere, partially-linked groups); `--fix` repairs the safe ones — including clearing orphan symlinks (cloud copies kept). |
| `dotsync undo [--list] [--dry-run] [--yes]` | Revert the most recent destructive run (`adopt` / `install --adopt` / `unadopt` / `group remove`): restore what it changed on this machine, skipping anything you've altered since. `--list` shows recent runs. Journaled per-machine in `~/.local/state/dotsync`. |
| `dotsync completions <shell>` | Print a shell completion script (setup installs these automatically). |
